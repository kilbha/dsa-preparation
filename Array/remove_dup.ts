...
function plusOne(digits: number[]): number[] {
    ...
    for (let i = digits.length - 1; i >= 0; i--) {
        RequiredNumber = RequiredNumber + digits[i] * 10 ** (digits.length - 1 - i);
        console.log("Required Number is ", RequiredNumber);
    }
    let targetNumber = RequiredNumber + 1;
    console.log("Target Number is ", targetNumber);
    let resultedARRAY = [];
    let numOfDigits = Math.floor(Math.log10(Math.abs(targetNumber)) + 1);
